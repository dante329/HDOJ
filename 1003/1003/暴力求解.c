...
//暴力运算优化O(n^2)
void find_max_sum_optimized(int n, int* arr, int* max_sum, int* start_pos, int* end_pos) {
    *max_sum = INT_MIN;  // 初始化最大和为负无穷
    for (int i = 0; i < n; i++) {        // 枚举子序列起点
        int current_sum = 0;            // 初始化当前子序列的和
        for (int j = i; j < n; j++) {   // 枚举子序列终点
            current_sum += arr[j];      // 累加当前终点的值
            if (current_sum > *max_sum) {  // 如果找到更大的和，更新结果
                *max_sum = current_sum;
                *start_pos = i + 1;     // 起始位置从 1 开始
                *end_pos = j + 1;       // 结束位置从 1 开始
            }
        }
    }
}
...
int main() {
    ...
    for (int case_num = 1; case_num <= t; case_num++) {
        ...
        find_max_sum_optimized(n, arr, &max_sum, &start_pos, &end_pos);
